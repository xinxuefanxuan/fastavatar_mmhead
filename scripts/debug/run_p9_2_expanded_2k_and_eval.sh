...
EXPERIMENT_NAME="p9_2i_expanded_2k"
CONFIG_PATH="${CONFIG_PATH:-configs/train/fastavatar_motion_zero_token_adapter_expanded_2k.yaml}"
...
SACRIFICIAL_VAL_ID="${P9_SACRIFICIAL_VAL_ID:-083}"
MAX_ITEMS_PER_ID="${P9_MAX_ITEMS_PER_ID:-8}"
DEBUG_STEPS="${P9_DEBUG_STEPS:-2000}"
METADATA_DIR="${P9_EXPANDED_METADATA_DIR:-outputs/mmhead_debug/p9_2_expanded_metadata}"
METADATA_PATH="${P9_EXPANDED_METADATA_PATH:-${METADATA_DIR}/${EXPERIMENT_NAME}_mixed_uids.json}"
RUNTIME_CONFIG_DIR="${P9_EXPANDED_RUNTIME_CONFIG_DIR:-outputs/mmhead_debug/p9_2_expanded_runtime_configs}"
RUNTIME_CONFIG_PATH="${RUNTIME_CONFIG_DIR}/fastavatar_motion_zero_token_adapter_expanded_2k_resolved.yaml"
EVAL_ROOT="${P9_EXPANDED_EVAL_ROOT:-outputs/mmhead_debug/p9_2_expanded_2k_eval}"
SENS_ROOT="${P9_EXPANDED_SENS_ROOT:-outputs/mmhead_debug/p9_2_expanded_2k_sensitivity}"
DIFF_ROOT="${P9_EXPANDED_DIFF_ROOT:-outputs/mmhead_debug/p9_2_expanded_2k_checkpoint_diff}"
CHECKPOINT_PATH="${P9_EXPANDED_CKPT:-exps/checkpoints/fastavatar/fastavatar_motion_zero_token_adapter_expanded_2k/002000/model.safetensors}"
...
PREFER_IDS="${TRAIN_IDS},${SACRIFICIAL_VAL_ID}"
...
echo "[P9.2I] Generate expanded metadata: ${P9_RESOLVED_METADATA}"
...
eval "$(python - "${CONFIG_PATH}" "${RUNTIME_CONFIG_PATH}" "${P9_RESOLVED_ROOT}" "${P9_RESOLVED_METADATA}" "${SACRIFICIAL_VAL_ID}" "${DEBUG_STEPS}" <<'PY'
from pathlib import Path
import json
import math
import shlex
...
for raw in config_path.read_text().splitlines():
    ...
    if stripped.startswith('child:') and not stripped.startswith('#'):
        raw = f'{indent}child: fastavatar_motion_zero_token_adapter_expanded_2k'
...
echo "[P9.2I] train_count=${P9_TRAIN_COUNT} steps_per_epoch=${P9_STEPS_PER_EPOCH} calculated_epochs=${P9_CALCULATED_EPOCHS} runtime_epochs=${P9_RUNTIME_EPOCHS}"
echo "[P9.2I] runtime_config=${P9_RESOLVED_RUNTIME_CONFIG}"

python FastAvatar/launch.py train.fastavatar --config "${P9_RESOLVED_RUNTIME_CONFIG}"

echo "[P9.2I] Run checkpoint diff"
...
for HOLDOUT_ID in "${HOLDOUT_ARRAY[@]}"; do
  echo "[P9.2I] Eval holdout ${HOLDOUT_ID}"
...
echo "[P9.2I] Adapter sensitivity on train split"
P9_NERSEMBLE_ROOT="${P9_RESOLVED_ROOT}" \
P9_SOURCE_META="${P9_RESOLVED_SOURCE}" \
...
for HOLDOUT_ID in "${HOLDOUT_ARRAY[@]}"; do
  echo "[P9.2I] Adapter sensitivity holdout ${HOLDOUT_ID}"
  P9_NERSEMBLE_ROOT="${P9_RESOLVED_ROOT}" \
  P9_SOURCE_META="${P9_RESOLVED_SOURCE}" \
...
echo "[P9.2I] Write summary report"
...
for holdout in holdouts:
    ...
    lines += [
        '| Variant | r_pixel mean | r_pixel std |',
        '|---|---:|---:|',
    ]
    for name, stats in variants.items():
        pixel = stats.get('r_pixel', {})
        lines.append(f'| {name} | {pixel.get("mean")} | {pixel.get("std")} |')
    lines += [
...
summary.write_text('\n'.join(lines) + '\n')
print(f'[P9.2I] wrote summary: {summary}')
PY

echo "[P9.2I] Done. Summary: ${EVAL_ROOT}/summary_report.md"
